import { useNavigate } from "react-router-dom";
import { useEffect, useRef } from "react";
import Navbar from "../components/Navbar";
import { googleLogin } from "../services/professionalsService";
import { saveAuthTokens, getAccessToken } from "../services/auth";

function Login() {
  const navigate = useNavigate();
  const hiddenButtonRef = useRef(null);

  useEffect(() => {
    if (getAccessToken()) {
      navigate("/settings");
    }
  }, [navigate]);

  async function handleCredentialResponse(response) {
    try {
      const result = await googleLogin(response.credential, "professional");
      saveAuthTokens(result.access_token, result.refresh_token);
      navigate("/explore");
    } catch (err) {
      console.error(err);
      alert("Não foi possível entrar com Google. Tente novamente.");
    }
  }

  useEffect(() => {
    const checkGoogleLoaded = setInterval(() => {
      if (window.google && window.google.accounts) {
        clearInterval(checkGoogleLoaded);
        window.google.accounts.id.initialize({
          client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID,
          callback: handleCredentialResponse,
        });
        window.google.accounts.id.renderButton(hiddenButtonRef.current, {
          type: "standard",
          theme: "outline",
          size: "large",
        });
      }
    }, 200);

    return () => clearInterval(checkGoogleLoaded);
  }, []);

  const handleGoogleLogin = () => {
    const realButton = hiddenButtonRef.current?.querySelector("div[role=button]");
    if (realButton) realButton.click();
  };

  return (
    <div className="min-h-screen flex flex-col">
      <Navbar variant="minimal" />
      <main className="flex flex-1 w-full pt-20">
        <section
          className="hidden lg:flex lg:w-1/2 relative items-center justify-center p-12 overflow-hidden bg-cover bg-center"
          style={{ backgroundImage: "linear-gradient(rgba(88,28,166,0.75), rgba(88,28,166,0.85)), url(https://images.unsplash.com/photo-1521791136064-7986c2920216?w=1200&q=80)" }}        >
          <div className="absolute top-8 left-8 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full flex items-center gap-2 text-white text-sm font-semibold">
            <span className="w-2 h-2 bg-secondary-container rounded-full"></span>
            Membros Selecionados
          </div>
          <div className="relative z-10 max-w-lg text-white">
            <h2 className="text-5xl font-extrabold leading-tight tracking-tight mb-6">
              A casa dos <span className="text-secondary-container">melhores</span> profissionais.
            </h2>
            <p className="text-xl text-on-primary-container/90 leading-relaxed mb-8">
              Conectamos clientes aos melhores profissionais. Entre para a Conecta Bairro.
            </p>
            <div className="flex items-center gap-3">
              <div className="flex -space-x-3">
                {[1, 2, 3].map((n) => (
                  <img
                    key={n}
                    src={`https://randomuser.me/api/portraits/med/men/${n * 11}.jpg`}
                    alt=""
                    className="w-10 h-10 rounded-full border-2 border-white object-cover"
                    />
                ))}              
              </div>
              <span className="text-sm font-medium">+2.400 Especialistas ativos hoje</span>
            </div>
          </div>
        </section>

        <section className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-24 bg-surface-container-lowest">
          <div className="w-full max-w-md">
            <div className="mb-10">
              <h1 className="text-3xl font-bold text-on-surface mb-2">Bem vindo, profissional!</h1>
              <p className="text-on-surface-variant">Ofereça serviços e encontre profissionais em um só lugar.</p>
            </div>

            <div ref={hiddenButtonRef} style={{ display: "none" }}></div>
            <button
              onClick={handleGoogleLogin}
              className="flex items-center justify-center px-4 py-3 border border-outline-variant/30 rounded-xl hover:bg-surface-container-low transition-colors w-full cursor-pointer"
            >
              <svg className="w-5 h-5 mr-3" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              <span className="text-sm font-semibold text-on-surface">Continuar com Google</span>
            </button>

            <button onClick={() => navigate("/")} className="mt-6 text-sm text-primary font-semibold hover:underline w-full text-center cursor-pointer">
              ← Voltar para a Home
            </button>

            <div className="mt-8 flex justify-center gap-4 text-xs text-on-surface-variant">
              <a href="#" className="hover:underline">Política de Privacidade</a>
              <span>·</span>
              <a href="#" className="hover:underline">Termos de Serviço</a>
              <span>·</span>
              <a href="#" className="hover:underline">Ajuda</a>
            </div>
          </div>
        </section>
      </main>
    </div>
  );
}

export default Login;